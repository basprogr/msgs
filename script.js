...
// Validasi parameter URL dasar
if (!currentUser || !roomId) {
  document.addEventListener("DOMContentLoaded", () => {
    showError("Akses ditolak: Parameter URL tidak lengkap.");
  });
} 
...
async function detectTarget() {
  ...
  if (data && data.length > 0) {
    const found = data.find(m => m.user_name !== currentUser);
    if (found) {
      targetName = found.user_name;
      userDisplay.textContent = targetName;
       
      if (targetName.length % 2 === 0) {
  ...
  supabaseClient
    ...
    .on(
      ...
      async (payload) => {
        ...
        // Update deteksi target & foto profil secara live jika ada pesan baru masuk
        await detectTarget();
